...
import org.banki.util.BigInteger;
import org.banki.util.StringUtils;
...
public class AESEncryption
{
  ...
  private byte[] decryptBytes(byte[] out) throws DataLengthException, IllegalStateException, InvalidCipherTextException {
    String magic = new String(org.banki.util.Arrays.slice(out, 0, 4));
    if ("BIE1".equals(magic)) {
      byte[] pre = org.banki.util.Arrays.slice(out, 4, out.length);
