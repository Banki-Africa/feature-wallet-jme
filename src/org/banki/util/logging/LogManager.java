...
package banki.util.logging;

import banki.beans.PropertyChangeListener;
import banki.beans.PropertyChangeSupport;
import banki.io.BufferedInputStream;
import banki.io.File;
import banki.io.FileInputStream;
import banki.io.IOException;
import banki.io.InputStream;
import banki.util.Collection;
import banki.util.Enumeration;
import banki.util.Hashtable;
import banki.util.Properties;
import banki.util.StringTokenizer;
 ...
 * banki.util.logging.manager system property, if the property is unavailable or
 * invalid, the default class {@link banki.util.logging.LogManager} will
 * be used.
 * <p>
 ...
 * <ul>
 * <li>"banki.util.logging.config.class"</li>
 * <li>"banki.util.logging.config.file"</li>
 * </ul>
 * <p>
 ...
 * <p>
 * The "banki.util.logging.config.class" should specifies a class name. If it is
 ...
 * {@code LogManager}.
 * <p>
 * If "banki.util.logging.config.class" property is not set, or it is invalid, or
 * some exception is thrown during the instantiation, then the
 * "banki.util.logging.config.file" system property can be used to specify a
...
public class LogManager {
     ...
     */
    public static final String LOGGING_MXBEAN_NAME = "banki.util.logging:type=Logging";
    ...
    static {
        ...
        String className = System.getProperty("banki.util.logging.manager");
        if (className != null) {
    ...
    public void readConfiguration() throws IOException {
        // check config class
        String configClassName = System.getProperty("banki.util.logging.config.class");
        if (configClassName == null || getInstanceByClass(configClassName) == null) {
            // if config class failed, check config file
            String configFile = System.getProperty("banki.util.logging.config.file");

            if (configFile == null) {
                // if cannot find configFile, use default logging.properties
                configFile = System.getProperty("banki.home") + File.separator + "lib" +
